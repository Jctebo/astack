...
Thanks for wanting to make astack better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

astack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/astack/` (your global install). But when you're developing astack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone <repo> && cd astack
...
Contributor mode turns astack into a self-improving tool. Enable it and Claude Code
will periodically reflect on its astack experience — rating it 0-10 at the end of
each major workflow step. When something isn't a 10, it thinks about why and files
a report to `~/.astack/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
~/.claude/skills/astack/bin/astack-config set astack_contributor true
```

The logs are for **you**. When something bugs you enough to fix, the report is
already written. Fork astack, symlink your fork into the project where you hit
the issue, fix it, and open a PR.

### The contributor workflow

1. **Use astack normally** — contributor mode reflects and logs issues automatically
2. **Check your logs:** `ls ~/.astack/contributor-logs/`
3. **Fork and clone astack** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where astack annoyed you)
   ln -sfn /path/to/your/astack-fork .claude/skills/astack
   cd .claude/skills/astack && bun install && bun run build
   ```
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using astack** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix astack while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ astack sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all 15 skills.

## Working on astack inside the astack repo

When you're editing astack skills and want to test them by actually using astack
...
```
astack/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── astack -> ../../         <- symlink back to repo root
│   ├── review -> astack/review
│   ├── ship -> astack/ship
...
When E2E tests run, they produce machine-readable artifacts in `~/.astack-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.astack-dev/` for post-mortem debugging and trend analysis.
...
**This is the recommended way to develop astack.** Symlink your astack checkout
into the project where you actually use it, so your changes are live while you
do real work:

```bash
# In your core project
ln -sfn /path/to/your/astack-checkout .claude/skills/astack
cd .claude/skills/astack && bun install && bun run build
```
...
```bash
rm .claude/skills/astack
```

Claude Code falls back to `~/.claude/skills/astack/` automatically.
