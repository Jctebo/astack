...
describe('validateOutputPath', () => {
  it('allows paths within TEMP_DIR', () => {
    expect(() => validateOutputPath(`${TEMP_DIR}/screenshot.png`)).not.toThrow();
  });

  it('allows paths in subdirectories of TEMP_DIR', () => {
    expect(() => validateOutputPath(`${TEMP_DIR}/browse/output.png`)).not.toThrow();
  });
  ...
  it('blocks TEMP_DIR prefix collision', () => {
  ...
  it('blocks path traversal via ..', () => {
    expect(() => validateOutputPath(`${TEMP_DIR}/../etc/passwd`)).toThrow(/Path must be within/);
  });
});

describe('validateReadPath', () => {
  it('allows absolute paths within TEMP_DIR', () => {
    expect(() => validateReadPath(`${TEMP_DIR}/script.js`)).not.toThrow();
  });
  ...
  it('blocks TEMP_DIR prefix collision', () => {
    expect(() => validateReadPath(`${TEMP_DIR}-evil/file.js`)).toThrow(/Absolute path must be within/);
  });
