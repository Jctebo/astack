...
describe('isPathWithin', () => {
  test('path inside directory returns true', () => {
    expect(isPathWithin(`${TEMP_DIR}/foo`, TEMP_DIR)).toBe(true);
  });

  test('path outside directory returns false', () => {
    expect(isPathWithin(`${process.cwd()}/foo`, TEMP_DIR)).toBe(false);
  });
  ...
  test('partial prefix does not match (path traversal)', () => {
    expect(isPathWithin(`${TEMP_DIR}-evil/foo`, TEMP_DIR)).toBe(false);
  });

  test('nested path returns true', () => {
    expect(isPathWithin(`${TEMP_DIR}/a/b/c`, TEMP_DIR)).toBe(true);
  });
});
