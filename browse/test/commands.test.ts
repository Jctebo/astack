...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/astack-cli-');
    const execPath = path.join(root, '.claude/skills/astack/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/astack/browse/src/server.ts');
