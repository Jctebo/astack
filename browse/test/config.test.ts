...
function createTempGitRepo(): string {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'browse-config-git-'));
  const init = Bun.spawnSync(['git', 'init', tmpDir], { stdout: 'pipe', stderr: 'pipe' });
  if (init.exitCode !== 0) {
...
describe('config', () => {
  describe('getGitRoot', () => {
    test('returns a path when in a git repo', () => {
      ...
      try {
        const root = withCwd(tmpDir, () => getGitRoot());
  ...
  describe('resolveConfig', () => {
    test('uses git root by default', () => {
      const tmpDir = createTempGitRepo();
      try {
        const config = withCwd(tmpDir, () => resolveConfig({}));
        expect(path.normalize(config.projectDir)).toBe(path.normalize(tmpDir));
        expect(path.normalize(config.stateDir)).toBe(path.normalize(path.join(tmpDir, '.astack')));
        expect(path.normalize(config.stateFile)).toBe(path.normalize(path.join(tmpDir, '.astack', 'browse.json')));
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    });
    ...
    test('derives paths from BROWSE_STATE_FILE when set', () => {
      const stateFile = '/tmp/test-config/.astack/browse.json';
      const config = resolveConfig({ BROWSE_STATE_FILE: stateFile });
      expect(config.stateFile).toBe(stateFile);
      expect(config.stateDir).toBe('/tmp/test-config/.astack');
  ...
  describe('ensureStateDir', () => {
    test('creates directory if it does not exist', () => {
      ...
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.astack', 'browse.json') });
    ...
    test('is a no-op if directory already exists', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-config-test-${Date.now()}`);
      const stateDir = path.join(tmpDir, '.astack');
    ...
    test('adds .astack/ to .gitignore if not present', () => {
    ...
    test('does not duplicate .astack/ in .gitignore', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n.astack/\n');
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.astack', 'browse.json') });
    ...
    test('logs warning to browse-server.log on non-ENOENT gitignore error', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      // Create a read-only .gitignore (no .astack/ entry → would try to append)
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n');
  ...
  describe('getRemoteSlug', () => {
    test('returns owner-repo format for current repo', () => {
      const tmpDir = createTempGitRepo();
      try {
        const addRemote = Bun.spawnSync(['git', '-C', tmpDir, 'remote', 'add', 'origin', 'git@github.com:example/astack.git'], {
          stdout: 'pipe',
          stderr: 'pipe',
        });
        if (addRemote.exitCode !== 0) {
          throw new Error(`git remote add failed: ${addRemote.stderr.toString()}`);
        }
        const slug = withCwd(tmpDir, () => getRemoteSlug());
        expect(slug).toBe('example-astack');
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    });
    ...
    test('parses SSH remote URLs', () => {
      ...
      const url = 'git@github.com:Jctebo/astack.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('Jctebo-astack');
    });

    test('parses HTTPS remote URLs', () => {
      const url = 'https://github.com/Jctebo/astack.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('Jctebo-astack');
    });
    ...
    test('parses HTTPS remote URLs without .git suffix', () => {
      const url = 'https://github.com/Jctebo/astack';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('Jctebo-astack');
    });
