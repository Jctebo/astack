...
import { chmodSync, copyFileSync, mkdtempSync, writeFileSync, rmSync, existsSync, readFileSync, mkdirSync, utimesSync } from 'fs';
import { join } from 'path';
import { tmpdir } from 'os';

const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'astack-update-check');
const BASH = process.platform === 'win32' && existsSync('C:\\Program Files\\Git\\bin\\bash.exe')
  ? 'C:\\Program Files\\Git\\bin\\bash.exe'
  : 'bash';

let astackDir: string;
let stateDir: string;
...
beforeEach(() => {
  astackDir = mkdtempSync(join(tmpdir(), 'astack-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'astack-state-test-'));
  // Copy the real astack-config so the test works on Windows without symlink privileges.
  const binDir = join(astackDir, 'bin');
  mkdirSync(binDir);
  const astackConfig = join(binDir, 'astack-config');
  copyFileSync(join(import.meta.dir, '..', '..', 'bin', 'astack-config'), astackConfig);
  chmodSync(astackConfig, 0o755);
});

afterEach(() => {
  rmSync(astackDir, { recursive: true, force: true });
...
function writeVersion(version: string) {
  const releasesDir = join(astackDir, 'docs', 'releases');
  mkdirSync(releasesDir, { recursive: true });
  writeFileSync(join(releasesDir, 'VERSION'), `${version}\n`);
}
...
describe('astack-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    const releasesDir = join(astackDir, 'docs', 'releases');
    mkdirSync(releasesDir, { recursive: true });
    writeFileSync(join(releasesDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('re-checks when UP_TO_DATE cache version does not match local', () => {
    ...
    writeFileSync(join(astackDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('re-checks when local version does not match cached old version', () => {
    ...
    writeFileSync(join(astackDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeVersion('0.3.3');
    writeFileSync(join(astackDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    const versionFile = join(projectRoot, 'docs', 'releases', 'VERSION');
