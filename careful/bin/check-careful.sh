...
if [ -z "$CMD" ]; then
  ...
  if [ -z "$CMD" ] && command -v node >/dev/null 2>&1; then
    CMD=$(printf '%s' "$INPUT" | node -e 'const fs = require("fs"); try { const data = JSON.parse(fs.readFileSync(0, "utf8")); console.log(data?.tool_input?.command || ""); } catch { process.stdout.write(""); }' 2>/dev/null || true)
  fi
  if [ -z "$CMD" ]; then
    for NODE_CANDIDATE in "/c/Program Files/nodejs/node.exe" "/c/Program Files (x86)/nodejs/node.exe" "/mnt/c/Program Files/nodejs/node.exe"; do
      if [ -x "$NODE_CANDIDATE" ]; then
        CMD=$(printf '%s' "$INPUT" | "$NODE_CANDIDATE" -e 'const fs = require("fs"); try { const data = JSON.parse(fs.readFileSync(0, "utf8")); console.log(data?.tool_input?.command || ""); } catch { process.stdout.write(""); }' 2>/dev/null || true)
        [ -n "$CMD" ] && break
      fi
    done
  fi
...
if [ -n "$WARN" ]; then
  ...
  mkdir -p ~/.astack/analytics 2>/dev/null || true
  echo '{"event":"hook_fire","skill":"careful","pattern":"'"$PATTERN"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}' >> ~/.astack/analytics/skill-usage.jsonl 2>/dev/null || true
