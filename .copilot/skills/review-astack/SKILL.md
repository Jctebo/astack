...
You are running the `/review-astack` workflow. Analyze the current branch's diff against the base branch for structural issues that tests don't catch.
...
1. Read the active release artifact in `docs/releases/` and use its `Plan` and `Progress` sections if they exist. Also read `TODOS.md` (if it exists), the PR description (`gh pr view --json body --jq .body 2>/dev/null || true`), and commit messages (`git log origin/<base>..HEAD --oneline`).
   **If no PR exists:** rely on commit messages and TODOS.md for stated intent — this is the common case since /review-astack runs before /ship-astack creates the PR.
...
Read `.copilot/skills/review/checklist.md`.
...
Read `.copilot/skills/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
```bash
source <(~/.copilot/skills/astack/bin/astack-diff-scope <base> 2>/dev/null)
```
...
2. **Read `.copilot/skills/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
~/.copilot/skills/astack/bin/astack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
2. If the doc file was NOT updated in this branch but the code it describes WAS changed, flag it as an INFORMATIONAL finding:
   "Documentation may be stale: [file] describes [feature/component] but code changed in this branch. Consider running `/document-release-astack`."

This is informational only — never critical. The fix action is `/document-release-astack`.
...
CODEX_REVIEWS_CFG=$(~/.copilot/skills/astack/bin/astack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.copilot/skills/astack/bin/astack-config set codex_reviews enabled
```
...
```bash
~/.copilot/skills/astack/bin/astack-config set codex_reviews disabled
```
...
```bash
~/.copilot/skills/astack/bin/astack-review-log '{"skill":"codex-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
- **Fix-first, not read-only.** AUTO-FIX items are applied directly. ASK items are only applied after user approval. Never commit, push, or create PRs — that's /ship-astack's job.
