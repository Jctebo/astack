...
_TEL=$(~/.copilot/skills/astack/bin/astack-config get telemetry 2>/dev/null || true)
...
for _PF in ~/.astack/analytics/.pending-*; do [ -f "$_PF" ] && ~/.copilot/skills/astack/bin/astack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true; break; done
```
...
If A: run `~/.copilot/skills/astack/bin/astack-config set telemetry community`
...
If B→A: run `~/.copilot/skills/astack/bin/astack-config set telemetry anonymous`
If B→B: run `~/.copilot/skills/astack/bin/astack-config set telemetry off`
...
~/.copilot/skills/astack/bin/astack-telemetry-log \
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.copilot/skills/astack/browse/dist/browse" ] && B="$_ROOT/.copilot/skills/astack/browse/dist/browse"
[ -z "$B" ] && B=~/.copilot/skills/astack/browse/dist/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
