...
\`\`\`bash
_UPD=$(${ctx.paths.binDir}/astack-update-check 2>/dev/null || ${ctx.paths.localSkillRoot}/bin/astack-update-check 2>/dev/null || true)
...
for _PF in ~/.astack/analytics/.pending-*; do [ -f "$_PF" ] && ${ctx.paths.binDir}/astack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true; break; done
\`\`\``;
}
...
function generateReviewDashboard(_ctx: TemplateContext): string {
  return `## Plan And Review Readiness Dashboard
