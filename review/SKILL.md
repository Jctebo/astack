...
```bash
source <(~/.claude/skills/astack/bin/astack-diff-scope <base> 2>/dev/null)
```
...
```bash
~/.claude/skills/astack/bin/astack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
CODEX_REVIEWS_CFG=$(~/.claude/skills/astack/bin/astack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.claude/skills/astack/bin/astack-config set codex_reviews enabled
```
...
```bash
~/.claude/skills/astack/bin/astack-config set codex_reviews disabled
```
...
```bash
~/.claude/skills/astack/bin/astack-review-log '{"skill":"codex-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
