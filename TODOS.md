...
**Effort:** L
**Priority:** P2
**Depends on:** /setup-astack-upload, visual PR annotations
...
**Context:** Requires `ANTHROPIC_API_KEY` in CI secrets. Cost is ~$4/run. Eval persistence system (v0.3.6) writes JSON to `~/.astack-dev/evals/` — CI would upload as GitHub Actions artifacts and use `eval:compare` to post delta comment.
...
### Workflow docs → Supabase team store sync

**What:** Add astack workflow docs (`00-scope.md`, `01-research.md`, `02-plan.md`, `03-progress.md`) and design docs to the Supabase sync pipeline alongside retro snapshots and QA reports.

**Why:** Cross-team planning discovery at scale. The current repo-root workflow artifacts work well for a single repo, but shared sync would make scoped ideas, research, and implementation plans visible across the whole team.

**Context:** astack now writes its core planning artifacts into the repo root. The team store already syncs structured artifacts like QA reports and retros. This would extend the same pattern to scope, research, planning, progress, and design context.
...
**What:** Skill that helps founders prepare their YC application after `/scope` identifies strong signal. Pulls from `00-scope.md`, research context, and design docs, structures answers to YC app questions, and runs a mock interview.

**Why:** Closes the loop. `/scope` identifies the founder and the wedge, `/yc-prep` helps them apply well, and the scope plus research docs already contain much of the raw material for a strong application.

**Effort:** M (human: ~2 weeks / CC: ~2 hours)
**Priority:** P2
**Depends on:** scope founder-discovery flow maturing first

## Design Review

### Plan-mode design review consolidation — SHIPPED

The old standalone plan-mode design review flow has been folded into `/plan` in astack. `/design-review` and `/design-consultation` remain as the live-site and design-system companions, and `{{DESIGN_METHODOLOGY}}` still powers the shared design QA language.
