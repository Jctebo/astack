...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.astack/*" -not -path "./.context/*" | sort
```
...
(adapt to whatever project you're in — these are not astack-specific):
...
```bash
gh pr view --json body -q .body > /tmp/astack-pr-body-$$.md
```
...
```bash
gh pr edit --body-file /tmp/astack-pr-body-$$.md
```

5. Clean up the tempfile:

```bash
rm -f /tmp/astack-pr-body-$$.md
```
