...
- Upgrading astack to latest version → suggest /astack-upgrade
  ...
  2. Run: astack-config set proactive false
  ...
  1. Run: astack-config set proactive true
...
echo '{"skill":"astack","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.astack/analytics/skill-usage.jsonl 2>/dev/null || true
...
If `PROACTIVE` is `false`: do NOT proactively suggest other astack skills during this session.
Only run skills the user explicitly invokes. This preference persists across sessions via
`astack-config`.

# astack browse: QA Testing & Dogfooding
