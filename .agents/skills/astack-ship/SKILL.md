...
_TEL=$(~/.codex/skills/astack/bin/astack-config get telemetry 2>/dev/null || true)
...
for _PF in ~/.astack/analytics/.pending-*; do [ -f "$_PF" ] && ~/.codex/skills/astack/bin/astack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true; break; done
```

If `PROACTIVE` is `"false"`, do not proactively suggest astack skills — only invoke
them when the user explicitly asks. The user opted out of proactive suggestions.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.codex/skills/astack/astack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If `JUST_UPGRADED <from> <to>`: tell user "Running astack v{to} (just updated!)" and continue.
...
If A: run `~/.codex/skills/astack/bin/astack-config set telemetry community`
...
If B→A: run `~/.codex/skills/astack/bin/astack-config set telemetry anonymous`
If B→B: run `~/.codex/skills/astack/bin/astack-config set telemetry off`
...
~/.codex/skills/astack/bin/astack-telemetry-log \
...
4. Check planning and review readiness:

## Plan And Review Readiness Dashboard

Before shipping, check the canonical workflow artifacts and any available review
signals.
...
~/.codex/skills/astack/bin/astack-review-read 2>/dev/null || true
```
...
1. **Check for a prior override on this branch:**
   ```bash
   source <(~/.codex/skills/astack/bin/astack-slug 2>/dev/null)
   grep '"skill":"ship-review-override"' ~/.astack/projects/$SLUG/$BRANCH-reviews.jsonl 2>/dev/null || echo "NO_OVERRIDE"
   ```
...
2. **If no override exists,** use AskUserQuestion:
   ...
   - For Design Review: run `source <(~/.codex/skills/astack/bin/astack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (design-review-lite or `/design-review`) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
1. Read `.agents/skills/astack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
```bash
source <(~/.codex/skills/astack/bin/astack-diff-scope <base> 2>/dev/null)
```
...
2. **Read `.agents/skills/astack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
~/.codex/skills/astack/bin/astack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
Read `.agents/skills/astack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
Read `.agents/skills/astack/review/TODOS-format.md` for the canonical format reference.
